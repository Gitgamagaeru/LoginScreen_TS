import { loginWithCredentials, loginWithGoogle, loginWithGithub } from "@/lib/actions"
      ...
      <div className="w-full max-w-md">
        ...
        {error && (
          <div className="mb-6 p-4 rounded-xl bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 text-sm font-medium border border-red-200 dark:border-red-500/20 animate-fade-in-up">
            {error === "CredentialsSignin" ? "メールアドレスまたはパスワードが正しくありません。" : "ログイン処理中にエラーが発生しました。"}
          </div>
        )}
          ...
          <div className="mt-8">
            ...
            <div className="mt-6 grid grid-cols-2 gap-4">
              ...
              <form action={loginWithGoogle}>
                <button type="submit" className="flex items-center justify-center w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-sm font-medium transition-colors shadow-sm active:scale-95 duration-200">
        ...
        <p className="text-center text-sm text-slate-500 dark:text-slate-400">
          アカウントをお持ちでない場合は{' '}
          <Link href="/register" className="font-semibold text-indigo-600 hover:text-indigo-500 dark:text-indigo-400 hover:underline transition-colors">
            無料登録
          </Link>
        </p>
      </div>
